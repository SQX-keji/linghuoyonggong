	...
	<view>
		...
		<view class="search flex justify-center align-center">
			<view class="search-box flex align-center justify-between">
				...
				<view @click="go('/my/message/workSet')" class="workTips flex">

					<u-switch style="margin-bottom:10rpx" :typeK="false" 
					v-model="checked" size="36" ></u-switch>
		...
		<u-sticky offset-top="-80" :enable="enable" @fixed="isTop = true" @unfixed="isTop = false"
			...
			<view class="list flex align-center justify-center" v-for="(item,index) in list" :key="index">
				<view class="list-box">
					<view @click="goDet(item.postPushId)" class="list-box-title flex align-center justify-between">
						<view class="list-box-title-l flex align-center">
							<view v-if="item.isTop==1" style="background:#FF4D50;" class="list-box-title-l-label">
								置顶
							</view>
							<view v-if="item.userType==2" style="background:#FF4D50;" class="list-box-title-l-label">
								企业
							</view>
							<view class="list-box-title-l-label" style="background:#6CAEFF">
								{{item.workType}}
							</view>
							{{item.ruleClassifyName}}
						</view>
					...
					<view @click="goDet(item.postPushId)" class="list-box-content">
						{{item.content}}
					</view>
					<view class="time">
						{{ item.createTime }}
						<!-- {{ $u.timeFrom(item.createTime,false)}} -->
					</view>
			...
			<u-popup :mask-close-able="false" :closeable="true" v-model="showShare" mode="center" width="686"
				...
				<view class="list-box">
					<view class="list-box-title flex align-center justify-between">
						<view class="list-box-title-l flex align-center">
							<view v-if="shareInfo.isTop==1" style="background:#FF4D50;" class="list-box-title-l-label">
								置顶
							</view>
							<view class="list-box-title-l-label" style="background:#6CAEFF">
								{{shareInfo.workType}}
					...
					<view class="list-box-btn flex align-center justify-between">
						<view
							...
							class="list-box-btn-r flex align-center justify-center" style="width: 100%;">
							<image style="width: 46rpx;height: 46rpx;margin-right: 20rpx;"
								src="/static/image/index/phone.png" mode=""></image>
							电话联系
						</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			//h5/app分享
			shares(postPushId) {
				let url = config.APIHOST2 + '/pages/index/index?invitation=' + uni.getStorageSync('invitationCode') +
			...
			//获取招聘首页轮播图
			getBannerList() {
				this.$Request.getT('/app/banner/selectBannerList?state=1&classify=1').then(res => {
					if (res.code === 0) {
			...
			goDet(postPushId) {
				if (uni.getStorageSync('token')) {
					...
					// #endif
					uni.navigateTo({
						url: '/pages/jobhunt/detail?postPushId=' + postPushId
					})
				} else {
					this.noLogin()
				}
			...
			getMsgData() {
				this.$Request.getT("/app/pushMsgType/getPushMsgTypeByUser", {
					user: 1
				}).then(res => {
					console.log(res);
					...
					if (res.code == 0 && res.data) {
						...
						this.checked = res.data.isEnable==1
